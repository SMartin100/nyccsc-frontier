...
$sql = "SELECT a.facilityna AS name, a.owner, public.ST_AsGeoJSON(public.ST_Transform(ST_GeometryN(a.geom,1),4326),6) as geojson FROM water.spdes AS a;";

# Select only municipal
#   $sql = "SELECT a.facilityna AS name, a.owner, public.ST_AsGeoJSON(public.ST_Transform(ST_GeometryN(a.geom,1),4326),6) as geojson FROM water.spdes AS a
#    WHERE a.OWNER ilike ANY(ARRAY['TOWN%', 'VILLAGE%','%CITY%']);";
